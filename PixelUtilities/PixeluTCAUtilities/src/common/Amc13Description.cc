...
{
  delete fTrigger;
  for (std::vector<BGO*>::iterator it = fBGOs.begin(); it != fBGOs.end(); ++it) delete *it;
  fBGOs.clear();
  fAMCMask.clear();
  fT1map.clear();
  fT2map.clear();
...
uint32_t Amc13Description::getReg( int pTounge, std::string& pReg )
{
  ...
  if (cReg == cMap.end()) std::cout << "The AMC13 does not have a register " << pReg << " in the memory map for Tounge " << pTounge << std::endl;
...
void Amc13Description::setReg( int pTounge, std::string& pReg, uint32_t pValue)
{
  RegMap cMap = (pTounge == 1) ? fT1map : fT2map;
  auto cReg = cMap.find(pReg);
  if ( cReg == cMap.end() ) cMap.insert( {pReg, pValue} );
