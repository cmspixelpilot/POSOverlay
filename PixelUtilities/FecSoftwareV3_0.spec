%define _package FecSoftwareV3_0
%define _packagename FecSoftwareV3_0
%define _version __version__
%define _release __build__
%define _prefix  /opt/xdaq
%define _tmppath /tmp
%define _packagedir $BUILD_HOME/FecSoftwareV3_0
%define _os linux
...
mkdir -p $RPM_BUILD_ROOT/%{_prefix}/dat/%{_package}/dat
...
if [ -d %{_packagedir}/lib ]; then
	install -m 655 %{_packagedir}/lib/* $RPM_BUILD_ROOT/%{_prefix}/lib/
fi
...
if [ -d %{_packagedir}/config ]; then
	cd %{_packagedir}/config; \
	find ./ -name "*.dat" -exec install -m 655 -D {} $RPM_BUILD_ROOT/%{_prefix}/dat/%{_package}/{} \;
fi
