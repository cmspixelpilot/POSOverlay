...
PixelAnalyzer::PixelAnalyzer(
	...
	PixelConfigurationsManager *pixelConfigurationsManager, ostream *logger)
{
	logger_ = logger;
...
void PixelAnalyzer::loopOverDataFile(int nEventsToProcess)
{
	...
	unsigned int numberOfTriggersPerPattern =
		thePixelCalib_->nTriggersPerPattern();
	*logger_ << mthn << "Ntriggers per pattern=" << numberOfTriggersPerPattern
			 << endl;
	...
	*logger_ << mthn << " # scan points=" << numberOfVCalPerPattern << endl;
	...
	//  *logger_ << mthn << "VCal => min=" << vcalMin << " max=" << vcalMax  <<
	//  " step=" << vcalStep << endl;
	...
	multimap<string, unsigned int> theFileMap =
		thePixelConfigurationsManager_->getDataFileMap();
	unsigned int numberOfFiles = theFileMap.size();
	unsigned int numberOfPatterns =
		thePixelCalib_->nPixelPatterns() * numberOfFiles;
	...
	for (multimap<string, unsigned int>::iterator it = theFileMap.begin();
		 it != theFileMap.end(); ++it, ++fileNumber) {
		...
		if (!dataFile.is_open()) {
			cout << mthn << "Can't open file: " << it->first
				 << "...Check the file name for fed " << it->second << endl;
			continue;
		}
		...
		*logger_ << mthn << " Run Number " << runNumber.getWord() << endl;
		...
		unsigned int eventsInAPattern = numberOfTriggersPerPattern *
										numberOfVCalPerPattern *
										numberOfWBCPerPattern;
		...
		while (readEvent(sLinkDecoder, sLinkEvents) &&
			...
			if (eventNumber == 0) {
				for (unsigned int fedCounter = 0; fedCounter < numberOfFeds;
					 ++fedCounter) {
					firstCounterValues[fedCounter] =
						sLinkEvents[fedCounter].getHeader().getLV1_id();
				}
			}
			...
			for (unsigned int fedCounter = 0; fedCounter < numberOfFeds;
				...
				if (eventNumber % maxTriggerCounterValue !=
					(sLinkEvents[fedCounter].getHeader().getLV1_id() -
					 firstCounterValues[fedCounter]) %
						maxTriggerCounterValue) {
					*logger_
						<< mthn << "An event was missing...resyncing"
						<< " Event: " << eventNumber + 1 << " != LV1_id: "
						<< sLinkEvents[fedCounter].getHeader().getLV1_id() -
							   firstCounterValues[fedCounter] << endl;
					// This part doesn't work if there are more feds in the same
					// file...you need to do everthing for the other feds
			...
			if (newPattern) {
				...
				if (eventNumber != 0) {
					...
					thePixelHistoManager_->endOfPatternAnalysis(); // fit histos
																   // so far and
																   // fill
																   // summary
																   // plots
					...
					if (difftime(endPattern, lastTimeUpdate) >= 5) {
						...
						double timeLeft =
							(numberOfPatternsLeft *
							 (elapsedTime /
							  (numberOfPatterns - numberOfPatternsLeft)));
						...
						if (DEBUG)
							cout << mthn << "Each pattern takes "
								 ...
								 << difftime(endPattern, lastTimeUpdate)
								 << " delta: " << deltaPatterns
								 << " left: " << numberOfPatternsLeft << endl;
						deltaPatterns = 0;
						time(&lastTimeUpdate);
					}
					time(&startPattern);
			...
			if (numberOfVCalPerPattern > 1 &&
				eventNumber % (numberOfTriggersPerPattern) == 0) {
				// vcalValue=
				// thePixelCalib_->scanCounter("Vcal",eventNumber/numberOfTriggersPerPattern)*vcalStep+vcalMin;//SCurve
				// type histos
				vcalValue = thePixelCalib_->scanValue(
					"Vcal",
					eventNumber /
						numberOfTriggersPerPattern); // SCurve type histos
			}
			...
			if (eventNumber == 0) {
				for (unsigned int fedCounter = 0; fedCounter < numberOfFeds;
					...
					if (firstCounterValues[fedCounter] != 1) {
						*logger_
							<< mthn << "The first event of fed "
							<< sLinkEvents[fedCounter]
								   .getHeader()
								   .getSource_id()
							<< " has the counter that doesn't start from 1 but "
							   "it starts at "
							<< sLinkEvents[fedCounter].getHeader().getLV1_id()
							<< endl;
						eventNumber =
							sLinkEvents[fedCounter].getHeader().getLV1_id() - 1;
			...
			for (unsigned int fedCounter = 0; fedCounter < numberOfFeds;
				...
				for (; ihit != hits.end(); ++ihit) {
					...
					//-- could be made faster by figuring out in advance from
					//calib file which events to run on
					int desiredWBC =
						thePixelConfigurationsManager_
							->getWBCChoice(); // could move to constructor
					if (desiredWBC >= 0 && desiredWBC <= 255) {
						int thiswbc = thePixelCalib_->scanValue(
							"WBC", eventNumber / numberOfTriggersPerPattern);
						if (thiswbc != desiredWBC) {
							continue;
						}
					}
					...
					if (numberOfVCalPerPattern > 1) {
						...
						thePixelHistoManager_->fillHistos(
							fed, channel, roc, row, col, vcalValue,
							adc); // SCurve type histo
					} else {
						thePixelHistoManager_->fillHistos(
							fed, channel, roc, row, col); // PixelAlive type
					}
				}
		...
		thePixelHistoManager_->endOfPatternAnalysis(); // fit histos so far and
													   // fill summary plots
		dataFile.close();

		//     break;//TO BE REMOVED
		*logger_ << mthn << "...Processed:" << eventNumber << " triggers"
				 << endl;
	...
	cout << mthn << "Total time elapsed: " << totalMinutes
		 << "m:" << totalSeconds << "s!" << endl;

	thePixelHistoManager_
		->endOfFileAnalysis(); // fit histos so far and fill summary plots
}

////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool PixelAnalyzer::readEvent(SLinkDecoder &sLinkDecoder,
							  vector<PixelSLinkEvent> &sLinkEvents)
{
	for (vector<PixelSLinkEvent>::iterator it = sLinkEvents.begin();
		 it != sLinkEvents.end(); ++it) {
