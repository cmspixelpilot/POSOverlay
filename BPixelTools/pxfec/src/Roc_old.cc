...
void Roc::Execute(SysCommand *command){
  if(command->type==SysCommand::kROC){
    ...
    else if(command->Keyword("Vsf",       &value1)){ setDAC(0x03, *value1); }
    ...
    else if(command->Keyword("VOffsetR0", &value1)){ setDAC(0x11, *value1); }
    else if(command->Keyword("VIon",      &value1)){ setDAC(0x12, *value1); }
    else if(command->Keyword("VIbias_PH", &value1)){ setDAC(0x13, *value1); }
    else if(command->Keyword("Ibias_DAC", &value1)){ setDAC(0x14, *value1); }
