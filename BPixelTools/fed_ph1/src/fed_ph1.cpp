...
unsigned int ttcaddcsr0=0x100000+0x60;//Base is slot 2 shifted over, 0x60 is BGO register
...
VMEout(0x3,(LAD_SC +(is<<17)),4,d);
  ...
  printf("\n\nFirmwVers  V: %02d.%02d.%02d%02d\n\n",((d&0xff000000)>>24),((d&0xff0000)>>16),((d&0xff00)>>8),(d&0xff) );
  ...
  VMEin(0x3,READ_MA,4,&d);	// Read ModuleSerialNumber and pass to CenterChip
  printf("\n\nMA= %d\n",d&0x3f);
  //int bora(0);
...
//cout << "k[105] = " <<hex<< k[105] << endl;
  ...
  // I think these should go away - you do not have a TTCVI in the crate to send these settings to.
...
/////       VMEout(0x3,LAD_NC+0x1a0000,4,0x1ff); //ch 10 disabled
       ...
       VMEout(0x3,LAD_N+0x1c0000,4,0x700); // For Up Piggy mode register bit [11..8] = 5 (0 for ch28, 1 for ch29, 2 for ch30, 3 for ch3a, 4 for ch32, 5 for ch33, 6 for ch34, 7 for ch35, 8 for ch36)
       VMEout(0x3,LAD_S+0x1c0000,4,0x500); // For Down Piggy  mode register bit [11..8] = 5 (0 for ch28, 1 for ch29, 2 for ch30, 3 for ch3a, 4 for ch32, 5 for ch33, 6 for ch34, 7 for ch35, 8 for ch36)
              
       VMEout(0x3,LAD_N+0x1a8000,4,CHa_CHb_mux+0x2305); // ch selection for transparent mode: 5-fiber#12, 4-fiber#11, 3-fiber#10, 2-fiber#9, 1-fiber# 8, 0-fiber#7
