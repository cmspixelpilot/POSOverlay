				...
				*/
				...
				else if ((attributes[i] & 0x1e) == 0x0e)
				{
					...
					uncompressed_size =
						...
						(((unsigned int) GET_BYTE(long_index + 1L)) << 8L) |
...
if(DBug){cout<<stackctr<<" opcode stack stackptr "<<hex<<opcode<<dec<<" "<<stack[stack_ptr]<<" "<<stack_ptr<<endl;stackctr++;}
		switch (opcode)
		{
		case 0x00: /* NOP  */
			/* do nothing */
