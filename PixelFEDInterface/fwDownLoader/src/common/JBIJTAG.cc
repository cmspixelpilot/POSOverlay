...
/*		June 2007 -Added support for Multi-read, multi-write	*/
/*		          and reduced jtag instruction cycles		*/
...
//		jbi_jtag_io(0, 0, 0);	/* DRSHIFT */
		...
		tmsbuf[0]=1;tdibuf[0]=0;
		tmsbuf[1]=0;tdibuf[1]=0;
		tmsbuf[2]=0;tdibuf[2]=0;
		
			jbi_jtag_io_multiwrite(tmsbuf, tdibuf, 3);}
...
//		jbi_jtag_io(0, 0, 0);	/* DRSHIFT */
		...
		tmsbuf[0]=1;tdibuf[0]=0;
		tmsbuf[1]=1;tdibuf[1]=0;
		tmsbuf[2]=1;tdibuf[2]=0;
		tmsbuf[3]=0;tdibuf[3]=0;
		tmsbuf[4]=0;tdibuf[4]=0;
		
			jbi_jtag_io_multiwrite(tmsbuf, tdibuf, 5);}
		break;
...
if(tdo != NULL){
	if((&tdi[0])==(&tdo[0])){
        ...
        int tmsbuf[count];
        int tdibuf[count];
        unsigned int tdobuf[count];
        int read_tdobuf[count];

		for (i = 0; i < count; i++)
		{tmsbuf[i]=(i == count - 1);
		 tdibuf[i]=tdi[i >> 3] & (1 << (i & 7));
		 read_tdobuf[i]=(tdo != NULL);}
		jbi_jtag_io_multiread(tmsbuf, tdibuf,read_tdobuf,tdobuf,count);
//		for (i = 0; i < count; i++){
//		cout<<"TMS = "<<dec<<tmsbuf[i]<<" TDI = "<<dec<<tdibuf[i]<<" TDO = "<<dec<<tdobuf[i]<<endl;}
...
//post processing			
		...
		tdibuf[i]=tdi[i >> 3] & (1 << (i & 7));
			...
			jbi_jtag_io_multiwrite(tmsbuf, tdibuf, count);
...
//		jbi_jtag_io(0, 0, 0);	/* IRSHIFT */
		...
		tmsbuf[3]=0;tdibuf[3]=0;
			...
			jbi_jtag_io_multiwrite(tmsbuf, tdibuf, 4);}
		break;
...
//		jbi_jtag_io(0, 0, 0);	/* IRSHIFT */
		...
		tmsbuf[1]=1;tdibuf[1]=0;
		tmsbuf[2]=1;tdibuf[2]=0;
		tmsbuf[3]=1;tdibuf[3]=0;
		tmsbuf[4]=0;tdibuf[4]=0;
		tmsbuf[5]=0;tdibuf[5]=0;
		
			jbi_jtag_io_multiwrite(tmsbuf, tdibuf, 6);}
		break;
...
{        int tmsbuf[jbi_dr_preamble];
         int tdibuf[jbi_dr_preamble];
		for (i = 0L; i < jbi_dr_preamble; i++)
		{tmsbuf[i]=(i == shift_count - 1);
		tdibuf[i]=(int) (jbi_dr_preamble_data[i >> 3L] & (1L << (i & 7L)))
		}
			jbi_jtag_io_multiwrite(tmsbuf, tdibuf, jbi_dr_preamble);
